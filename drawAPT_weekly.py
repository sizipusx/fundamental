...
def run_price_index(selected_dosi2, mdf, jdf, mdf_change, jdf_change):
    ...
    fig.update_layout(title = titles, titlefont_size=15, legend=dict(orientation="h"), template=template, xaxis_tickformat = '%Y-%m')
    fig.update_layout(template="myID")
    fig.add_vline(x="2019-1-14", line_dash="dash", line_color="gray", annotation_text="지수 기준시점", annotation_position="bottom right")
...
def run_one_index(selected_dosi2, omdf, ojdf, omdf_change, ojdf_change):
    ...
    fig.add_trace(go.Scatter(mode='lines', name = '매매지수', x =  omdf.index, y= omdf[selected_dosi2], marker_color = marker_colors[0]), secondary_y = False)
