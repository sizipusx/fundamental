...
def ecos_one_two_window(kor_exp, total_df):
    ...
    fig.add_hline(y=round(total_df['NBindex'].mean(),1), line_width=1, line_dash='solid', line_color=marker_colors1[1], annotation_text="명목평균: "+round(total_df['NBindex'].mean(),1), annotation_position="bottom right")
    fig.add_hline(y=round(total_df['RBindex'].mean(),1), line_width=1, line_dash='solid', line_color=marker_colors1[2], annotation_text="실질평균: "+round(total_df['RBindex'].mean(),1), annotation_position="bottom left")
    fig.update_layout(
                        showlegend=True,
                        legend=dict(
                                    orientation="h",
