...
def show_total(s_df):
    ...
    fig = px.scatter_mapbox(s_df, lat="위도", lon="경도",     color="주거형태", size="거래가(만원)", hover_name="단지명", hover_data=["물건수", "공급면적(평)", "시도명"],
...
def show_local(select_city, city_apt, city_total):
    px.set_mapbox_access_token(token)
    fig = px.scatter_mapbox(city_apt, lat="위도", lon="경도", color="주거형태", size="거래가(만원)", hover_name="단지명", hover_data=["물건수", "공급면적(평)", "시도명"],
