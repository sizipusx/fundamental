...
def run(ticker, overview_df, fdr_df):
    ...
    with tab1:
        ...
        try:
            ...
            col1, col2, col3, col4 = st.columns(4)
            col1.metric(label="현재 ROE 기준 매수가격", value = current_proper_price)
...
if __name__ == "__main__":
    ...
    fdr_df = yfin.download(input_ticker, start='2000-01-02',  multi_level_index=False)
    #st.dataframe(fdr_df)
    fdr_df['ma5'] = fdr_df.iloc[:,0].rolling(window=5).mean()
    fdr_df['ma20'] = fdr_df.iloc[:,0].rolling(window=20).mean()
    fdr_df['ma240'] = fdr_df.iloc[:,0].rolling(window=240).mean()
    # st.dataframe(ov_df)
    # st.dataframe(fdr_df)
    overview_df.loc['Close'] = round(fdr_df.iloc[-1,1],2)
