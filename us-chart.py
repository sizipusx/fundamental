...
def run(ticker):
    ...
    with tab1:
        ...
        col1.metric(label="현재 ROE 기준 기대수익률", value = round(y_df.iloc[-1,4]*100,2), delta=round((round(y_df.iloc[-1,4]*100,2)-expect_yield*100),2))
        col2.metric(label="최소 평균 기준 기대수익률", value =round(y_df.iloc[-1,3]*100,2), delta=round((round(y_df.iloc[-1,3]*100,2)-expect_yield*100),2))
        col3.metric(label="최소 평균 기준 기대수익률", value =round(y_df.iloc[-1,5]*100,2), delta=round((round(y_df.iloc[-1,5]*100,2)-expect_yield*100),2))
