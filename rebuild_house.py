...
def show_total(s_df):
    ...
    fig = px.scatter_mapbox(s_df, lat="위도", lon="경도",     color="주거형태", size="시세평균(만)", hover_name="단지명", hover_data=["물건수", "공급면적", "시도"],
                    color_continuous_scale=px.colors.cyclical.IceFire, height=600, size_max=30, zoom=10)
    fig.update_layout(
    ...
    if response:
        ...
        if selected_df:
            ...
            fig = px.scatter_mapbox(selected_df, lat="위도", lon="경도",     color="주거형태", size="공급면적", hover_name="단지명", hover_data=["특이사항", "한글거래가액", "시도"],
                            color_continuous_scale=px.colors.cyclical.IceFire, size_max=30, zoom=10, height=300)
            fig.update_layout(
