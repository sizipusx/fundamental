...
def run(ticker, com_name):
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric(label="기대수익률(RIM)", value = value_df.iloc[6])
    col2.metric(label="5년 연평균수익률", value =value_df.iloc[7])
    col3.metric("PBR갭수익률", value =value_df.iloc[8])
    col1, col2, col3 = st.columns(3)
    col1.metric(label="배당수익률(RIM)", value = value_df.iloc[9])
    col2.metric(label="시가수익률", value =value_df.iloc[10])
