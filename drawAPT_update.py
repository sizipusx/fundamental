...
def draw_mae_bs(selected_dosi, senti_dfs, df_as, df_bs):
    ...
    fig.add_trace(go.Scatter(line = dict(dash='dot'), name ='한산함', x =  js_4.index, y= js_4[selected_dosi], marker_color = marker_colors[1]), secondary_y = False)                                             
    fig.add_trace(go.Scatter(mode='lines', name ='매매거래지수', x =  js_sell.index, y= js_sell[selected_dosi], marker_color = '#34314c'), secondary_y = False)
...
def draw_jeon_bs(selected_dosi, senti_dfs, df_as, df_bs):
    ...
    fig.add_trace(go.Scatter(line = dict(dash='dot'), name ='수요<공급', x =  js_6.index, y= js_6[selected_dosi], marker_color = marker_colors[1]), secondary_y = False)                                             
    fig.add_trace(go.Scatter(mode='lines', name ='전세수급지수', x =  js_j.index, y= js_j[selected_dosi].round(2), marker_color = '#34314c'), secondary_y = False)
...
def draw_jeon_trade(selected_dosi, senti_dfs, df_as, df_bs):
    ...
    fig.add_trace(go.Scatter(line = dict(dash='dot'), name ='한산함', x =  js_8.index, y= js_8[selected_dosi], marker_color = marker_colors[1]), secondary_y = False)                                             
    fig.add_trace(go.Scatter(mode='lines', name ='전세거래지수', x =  js_js.index, y= js_js[selected_dosi], marker_color = '#34314c'), secondary_y = False)
...
def draw_kb_mfore(selected_dosi, senti_dfs, df_as, df_bs):
    ...
    fig.add_trace(go.Scatter(line = dict(dash='dot'), name ='약간하락', x =  js_10.index, y= js_10[selected_dosi], marker_color = marker_colors[1]), secondary_y = False)                                             
    fig.add_trace(go.Scatter(mode='lines', name ='매매가격 전망지수', x =  js_for.index, y= js_for[selected_dosi], marker_color = '#34314c'), secondary_y = False)
...
def draw_kb_jfore(selected_dosi, senti_dfs, df_as, df_bs):
    ...
    fig.add_trace(go.Scatter(line = dict(dash='dot'), name ='약간하락', x =  js_12.index, y= js_12[selected_dosi], marker_color = marker_colors[1]), secondary_y = False)                                             
    fig.add_trace(go.Scatter(mode='lines', name ='전세가격 전망지수', x =  js_for_j.index, y= js_for_j[selected_dosi], marker_color = '#34314c'), secondary_y = False)
...
def run_buy_index(selected_dosi, org_df):
    ...
    fig.add_hline(y=round(selected_df['합계'].mean(axis=0),1), line_width=1, line_color="blue", line_dash="dot", 
                        annotation_text="평균 거래량: "+str(round(selected_df['합계'].mean(axis=0),1)), annotation_position="bottom right")
    fig.update_layout(template="myID")
    st.plotly_chart(fig)
