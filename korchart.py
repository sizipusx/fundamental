...
def run(code, com_name):
    ...
    따라서 ‘좋은’ 펀드매니저는 이익 전망이 개선되는 기업, 다시 말해 기업의 이익예상 라이프사이클의 왼쪽 부분에 위치한 주식을 매수할 것이다.')
    gcycle = Image.open("growth.png")
    st.image(gcycle, caption='이익예상 라이프사이클에서의 투자자의 위치- **성장주**의 경우')
    st.markdown('투자자들이 성장주를 매수할 때 그들은 지금 자신이 다이아몬드를 구입했기를 기대한다. 바꿔 말하면 사람들이 많은 기대를 갖고 다이아몬드를 사는 것처럼 성장주 투자자는 매수한 주식에 대해 높은 기대 수준을 가지고 있다. 따라서 성장주 투자자는 이익예상 라이프사이클의 위쪽에 위치한다')
    vcycle = Image.open("value.png")
    st.image(vcycle, caption='이익예상 라이프사이클에서의 투자자의 위치- **가치주**의 경우')
    st.markdown('가치주 투자자들이 사과를 구입할 때 약간의 기대를 가지기는 하지만, 벌레가 있더라도 다소간의 충격은 있을지 몰라도 비극으로 받아들이지는 않는다. 즉, 가치주 투자자는 매입한 주식에 대해 큰 기대를 갖지 않는다. 따라서 가치주 투자자는 일반적으로 이익예상 라이프사이클의 아래쪽에 위치한다')
    st.image("image.png")
    totalcycle = Image.open(" 좋은 가치 VS 나쁜가치 VS  좋은 성장 VS 나쁜 성장")
