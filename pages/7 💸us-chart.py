...
def run(ticker, overview_df, fdr_df):
    ...
    with tab1:
        ...
        col1, col2, col3, col4 = st.columns(4)
        col1.metric(label="현재 ROE 기준 기대수익률", value = round(y_df.iloc[-1,5]*100,1), delta=round((round(y_df.iloc[-1,5]*100,1)-expect_yield),1))
        col2.metric(label="최소 ROE 기준 기대수익률", value =round(y_df.iloc[-1,4]*100,1), delta=round((round(y_df.iloc[-1,4]*100,1)-expect_yield),1))
        col3.metric(label="최대 ROE 기준 기대수익률", value =round(y_df.iloc[-1,6]*100,1), delta=round((round(y_df.iloc[-1,6]*100,1)-expect_yield),1))
        col4.metric(label="평균 ROE 기준 기대수익률", value =round(y_df.iloc[-1,7]*100,1), delta=round((round(y_df.iloc[-1,7]*100,1)-expect_yield),1))
        col1, col2, col3, col4 = st.columns(4)
