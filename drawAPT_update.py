...
marker_colors = ['rgb(160,103,173)', 'rgb(205,32,40)', 'rgb(22,108,150)', 'rgb(244,201,107)', 'rgb(153,204,0)', \
...
def run_jeon_ratio(selected_dosi, mr_df, ar_df):
    ...
    fig.add_trace(go.Scatter(mode='markers', 
                                    name = "중위전세가율", x =  mr_df.index, y=mr_df[selected_dosi].round(decimals=1),  
                                    text= mr_df[selected_dosi], textposition = 'top center', marker_color = marker_colors[0]),
                                    secondary_y = False)
    fig.add_trace(go.Scatter(mode='lines', 
                                    name = "평균전세가율", x =  ar_df.index, y=ar_df[selected_dosi].round(deciamls=1),  
...
def run_trade_index(selected_dosi, org_df, mdf):
    ...
    fig.add_trace(go.Scatter(mode='lines', 
                                    ...
                                    secondary_y = True)
    fig.update_traces(texttemplate='%{text:.3s}') 
    fig.add_hline(y=selected_df['합계'].mean(axis=0), line_width=1, line_color="blue", line_dash="dot", 
                        annotation_text=selected_df['합계'].mean(axis=0), annotation_position="bottom right")
