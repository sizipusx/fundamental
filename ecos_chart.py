...
def ecos_chart(input_ticker, df1, df2):
    ...
    with st.container():
        ...
        with col1:
            ...
            fig.update_yaxes(title_text='대출금액(조원)', range=[-max(df2.loc[:,y_data_line[0]]), max(df1.loc[:,y_data_bar[0]])*2], secondary_y = False)
        ...
        with col3: 
            ...
            for y_data, color in zip(y_data_line, marker_colors): 
                fig.add_trace(go.Scatter(mode='lines+markers+text', 
                                            ...
                                            secondary_y = False)
            #fig.update_traces(texttemplate='%{text:.3s}') 
            fig.update_yaxes(title_text='대출금액(조원)', range=[-max(df3.loc[:,y_data_line[0]]), max(df1.loc[:,y_data_bar[0]])*2], secondary_y = False)
            #fig.update_yaxes(title_text='Profit', range=[0, max(income_df.loc[:,y_data_bar[0]])*2], secondary_y = False)
