...
def pbr_chart(company_name, income_df, income_df_q):
    with st.container():
        ...
        with col1:
            ...
            fig.update_layout(title = titles, titlefont_size=15, legend=dict(orientation="h"), template=template)#, xaxis_tickformat = '%Y.%m')
            fig.add_hline(y=income_df.loc[:,'ROE'].fillna(0).mean(), line_width=2, line_dash="dot", line_color="red",  annotation_text=income_df.loc[:,'ROE'].fillna(0).mean(), annotation_position="bottom right")
            fig.update_layout(template="myID")
            st.plotly_chart(fig)
        with col2:
                st.write("")
